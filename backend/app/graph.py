...
def get_graph():
    ...
    Graph is created only on the first API request.
    """
    ...
    if _graph is None:
        ...
        # Import nodes only when graph is needed
        from app.nodes.analyze_request_node import (
        ...
        graph.add_node(
            "Introduction_Section_Generation",
            intro_RAG
        )

        graph.add_node(
            "Roadmap_Section_Generation",
            rm_RAG
        )

        graph.add_node(
            "Resources_Section_Generation",
            resources_RAG
        )

        graph.add_node(
            "Interview_Questions_Section_Generation",
            interview_RAG
        )

        graph.add_node(
            "Resume_Tips_Section_Generation",
            projects_RAG
        )
